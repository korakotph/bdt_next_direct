...
export default function PopupNews({ enabled }) {
  ...
  return (
    <div id="news-modal" className="fixed inset-0 bg-black/75 flex items-center justify-center z-[101] px-4">
      <div className="popup-swiper relative max-w-4xl w-full rounded-lg">
        ...
        <button className="swiper-prev custom-nav">‹</button>
                ...
                <div className="p-6 text-center space-y-4 relative">
                  {item.url && (
                    <a
                      href={item.url}
                      target="_blank"
                      className="inline-block px-4 py-2 bg-brand-500 text-white rounded absolute bottom-0 left-1/2 -translate-x-1/2"
                    >
                      {item.title}
                    </a>
                  )}
        ...
        <button className="swiper-next custom-nav">›</button>

        <button
          onClick={hideToday}
          className="block ml-auto text-white underline underline-offset-4 hover:text-brand-200 absolute bottom-2 right-0 cursor-pointer z-[999]"
        >
          ไม่แสดงอีกวันนี้
        </button>
      </div>
